...
def get_service_status(slots):
    ...
    speech_output = "I found " + str(len(instances[u'Tagged'])) + \
                    p.plural(" server", len(instances[u'Tagged'])) + \
                    " labelled as " + environment[u'name'] + ". "

    if len(instances[u'Online']) == len(instances[u'Tagged']):
        speech_output += "All of which are online."
    else:
        speech_output += "However " + str(len(instances[u'Offline'])) + " " + \
                         p.plural("server", len(instances[u'Offline'])) + \
                         p.plural(" is", len(instances[u'Offline'])) + " offline."
...
def set_service_status(slots):
    ...
    if state["id"] == "START":
        if instances[u'Offline']:
            ...
            speech_output = "I found " + str(len(instances[u'Offline'])) + " " + p.plural("server", len(
                instances[u'Offline'])) + " labelled as " + environment[u'name'] + ", I have now started " + p.plural(
                "it", len(instances[u'Offline']))
            send_sns_notification(speech_output)
        else:
        ...
        if instances[u'Online']:
            ...
            speech_output = "I found " + str(len(instances[u'Online'])) + " " + p.plural("server", len(
                instances[u'Online'])) + " labelled as " + environment[u'name'] + ", I have now stopped " + p.plural(
                "server", len(instances[u'Online']))
            send_sns_notification(speech_output)
        else:
...
def send_sns_notification(message):
    ...
    response = sns.publish(
        TopicArn='arn:aws:sns:us-east-1:657592885771:AWSAlexaDemoNotifications',
        Message=message
    )
